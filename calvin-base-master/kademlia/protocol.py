...
class KademliaProtocol(RPCProtocol):
    ...
    def call_find_node(self, node_to_ask, node_to_find):
        ...
        result = self.find_node(address, self.source_node.id,
                                      node_to_find.id)
        return self.handle_call_response(result, node_to_ask)

    def call_find_value(self, node_to_ask, node_to_find):
        address = (node_to_ask.ip, node_to_ask.port)
        result = self.find_value(address, self.source_node.id,
                                       node_to_find.id)
        return self.handle_call_response(result, node_to_ask)

    def call_ping(self, node_to_ask):
        address = (node_to_ask.ip, node_to_ask.port)
        result = self.ping(address, self.source_node.id)
        return self.handle_call_response(result, node_to_ask)

    def call_store(self, node_to_ask, key, value):
        address = (node_to_ask.ip, node_to_ask.port)
        result = self.store(address, self.source_node.id, key, value)
        return self.handle_call_response(result, node_to_ask)
