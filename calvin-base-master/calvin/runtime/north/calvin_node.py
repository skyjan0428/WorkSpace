...
# def peersetup_collect_cb(status, uri, peer_node_id, peer_node_ids, peers, org_cb):
#     if uri in peers:
#         pass
#     if not peers:
...
# def start(proxy_control_uri, control_uri):
#     if proxy_control_uri is not None:
#         pass #self.control.start(node=self, uri=proxy_control_uri, tunnel=True)
#     else:
...
# def start_node(security, uri,create_tracing_node, control_uri, trace_exec, attributes, create_node):
#     if not security:
#         pass
